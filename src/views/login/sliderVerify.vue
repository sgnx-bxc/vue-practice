<!--suppress CssUnusedSymbol, RequiredAttributes, HtmlRequiredAltAttribute, JSUnresolvedVariable -->
<template>
  <div
    class="slide-verify"
  ...
  >
    ...
    >
      ...
      <!--滑动条提示文字-->
      <span v-show="sliderButtonLeft === 0" class="slider-hint">{{
        sliderHint
      }}</span>
    </div>
  </div>
...
<script>
import { getVerifyImage } from '@/api/request/common'
function sum(x, y) {
  return x + y
}
  ...
  methods: {
    ...
    async getCaptcha() {
      const params = {
        ...
        blockWidth: this.blockWidth / 1.5,
        blockHeight: this.blockHeight,
        blockRadius: this.blockRadius,
        place: 1
      }
      try {
        const ret = await getVerifyImage(params)
        const data = ret.data.list[0]
        this.nonceStr = data.secretKey
        this.tokenCache = data.token
        this.$refs.block.src = data.jigsawImageBase64
        this.$refs.block.style.top = data.blockY + 'px'
        this.$refs.canvas.src = data.originalImageBase64
      } catch (e) {
        console.error(e)
      } finally {
    ...
    createImage(onload) {
      ...
      image.onload = onload
      image.src = require('@/assets/img/login/verirfyBg.jpg')
      return image
    },
    // 根据指定范围获取随机数
...
.img-loading {
  ...
  background-image: url('~@/assets/img/login/loading.svg');
...
.refresh-icon {
  ...
  height: 35px;
  cursor: pointer;
  background: url('~@/assets/img/login/light.png') 0 -432px;
...
.slider-button-icon {
  ...
  width: 15px;
  height: 13px;
  background: url('~@/assets/img/login/light.png') 0 -26px;
  background-size: 35px 470px;
}
